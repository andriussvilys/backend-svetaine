  ...
  constructor(props){
    ...
    onCheck = (e) => {
      ...
      const checkboxCheck = (checkbox, subcategory, e) => {
          ...
          this.setState({ category: {...this.state.category, [checkbox.value]: {} } })
        ...
        let subcategories = Object.keys(navData[category]).map(subcategory => {
          ...
          let listitems = navData[category][subcategory].map((listitem, index) => {
            return(
              <li key={`${listitem}${index}}`} className="list--listitem list-group-item">
                <input type="checkbox" value={listitem} id={listitem} onChange={(e) => {this.onCheck(e)}} />
                <span>{listitem}</span>  
              </li>
            )
          })
          return(
            <ul key={subcategory} value={subcategory} className="list--subcategory list-group list-group-item">
              <input type="checkbox" value={subcategory} onChange={(e) => {this.onCheck(e)}} />
    ...
    return(
      <div key={category} label={category} className="list-group">
        <ul value={category} className="list--category"> <input type="checkbox" value={category} onChange={(e) => {this.onCheck(e)}} /> <span>{category}</span>
        {subcategories}
        </ul>
      </div>
    )
  ...
  render(){
    return(
      <div>
        <ImageInfo />
        <div>
          <JsonPreview category={Object.keys(this.state.category)} state={this.state} 
